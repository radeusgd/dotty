...
object Strymonas {
  ...
  case class Stream[A](stream: StagedStream[Expr[A]]){
      inline def fold[W](z: W, f: (W, A) => W): W = ${ ???; Stream.foldImpl[A, W]('z, 'f, stream) }

      // inline def map[B](f: A => B): Stream[B] = ${ Strymonas.mapImpl('f) }
  }
  ...
  object Stream {
    ...
    /** Main consumer
      ...
      * @return
      */
    def foldImpl[A: Type, W: Type](z: Expr[W], f: Expr[(W, A) => W], stream: StagedStream[Expr[A]]): Expr[W] = {
      Var(z) { s: Var[W] => '{
          ${
