...
// TODO factor out common code with scala.collection.immutable.ArrayLike into scala.collection.ArrayLike
...
/** A common supertrait of `ArrayOps` and `WrappedArray` that factors out the
  ...
  *  @since   2.8
  */
trait IArrayLike[A, +Repr] extends Any with IndexedSeqOptimized[A, Repr] { self =>
  ...
  def deep: scala.collection.IndexedSeq[Any] = new scala.collection.AbstractSeq[Any] with scala.collection.IndexedSeq[Any] {
    ...
    def apply(idx: Int): Any = self.apply(idx) match {
      case x: AnyRef if x.getClass.isArray => WrappedIArray.make(x).deep
      case x => x
    }
    override def stringPrefix = "IArray"
  }
}
