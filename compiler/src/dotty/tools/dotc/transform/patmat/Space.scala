...
trait SpaceLogic {
   ...
   */
  def simplify(space: Space, aggressive: Boolean = false)(implicit ctx: Context): Space = trace(s"simplify ${show(space)}, aggressive = $aggressive --> ", show = true)(space match {
  ...
  def isSubspace(a: Space, b: Space)(implicit ctx: Context): Boolean = trace(s"${show(a)} < ${show(b)}", debug,  x => show(x.asInstanceOf[Space])) {
