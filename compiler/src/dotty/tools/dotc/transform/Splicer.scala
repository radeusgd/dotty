...
object Splicer {
  ...
  private class Interpreter(pos: SourcePosition, classLoader: ClassLoader)(implicit ctx: Context) extends AbstractInterpreter {
    ...
    protected def interpretQuote(tree: Tree)(implicit env: Env): Object =
      new scala.quoted.Exprs.TastyTreeExpr(Inlined(EmptyTree, Nil, tree).withSpan(tree.span), PickledQuotes.contextId)
    ...
    protected def interpretStaging()(implicit env: Env): Object =
      new StagingImpl {
        override val reflection = ReflectionImpl(ctx, pos)
      }
