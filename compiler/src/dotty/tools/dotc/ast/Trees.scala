...
object Trees {
  ...
  abstract class Instance[T >: Untyped <: Type] extends DotClass { inst =>
    ...
    abstract class TreeMap(val cpy: TreeCopier = inst.cpy) {
      ...
      def transform(tree: Tree)(implicit ctx: Context): Tree = {
        ...
        if (skipTransform(tree)) tree
          ...
          case Block(stats, expr) =>
            cpy.Block(tree)(transformStats(stats, tree.pos, expr.pos), transform(expr))
          ...
          case tree @ Template(constr, parents, self, _) =>
            cpy.Template(tree)(transformSub(constr), transform(parents), transformSub(self), transformStats(tree.body, tree.pos, NoPosition /*FIXME: not really correct*/))
          case Import(expr, selectors) =>
            cpy.Import(tree)(transform(expr), selectors)
          case PackageDef(pid, stats) =>
            cpy.PackageDef(tree)(transformSub(pid), transformStats(stats, tree.pos, pid.pos)(localCtx))
      ...
      def transformStats(trees: List[Tree], treePos: Position, ownerPos: Position)(implicit ctx: Context): List[Tree] =
        transform(trees)
