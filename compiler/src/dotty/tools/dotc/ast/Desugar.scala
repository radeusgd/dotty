...
// ----- Desugar methods -------------------------------------------------
  ...
  private def defDef(meth: DefDef, isPrimaryConstructor: Boolean = false)(implicit ctx: Context): Tree = {
    ...
    var meth1 = cpy.DefDef(meth)(tparams = tparams1, vparamss = addEvidenceParams(meth.vparamss, epbuf.toList))
  ...
  // Add all evidence parameters in `extraParams` to the list of method parameters `currentParamss` */
  private def addEvidenceParams(currentParamss: List[List[ValDef]], extraParams: List[ValDef])(implicit ctx: Context): List[List[ValDef]] =
    extraParams match {
      case Nil =>
        currentParamss
      case evidenceParams =>
        currentParamss.reverse match {
          case (vparams @ (vparam :: _)) :: rvparamss if vparam.mods is Implicit =>
            ((evidenceParams ++ vparams) :: rvparamss).reverse
          case _ =>
            currentParamss :+ evidenceParams
        }
    }
      ...
      def expandConstructor(tree: Tree) = tree match {
        case ddef: DefDef if ddef.name.isConstructorName =>
          decompose(
            defDef(
              cpy.DefDef(ddef)(
                tparams = constrTparams,
                vparamss = addEvidenceParams(ddef.vparamss, evidenceParams(constr1).map(toDefParam)))))
        case stat =>
          stat
      }
        ...
        val opsTrait = {
          ...
          def opMethods(tree: Tree): List[DefDef] = defTree(tree) match {
            ...
            case tree @ DefDef(_, _, (selfParam :: rest1) :: rest2, _, _) if !(tree.mods.flags is (Private | Protected | Override)) =>
              ...
              // BUT ALSO: F[A => B] ~ F[C] ~>  C <:< A => B

              def usesTparams(tree: Tree): Boolean =
                new UntypedDeepFolder[Boolean]({
                  case (acc, tree: Ident) =>
                    acc || tparamNames.contains(tree.name)
                  case (acc, _) =>
                    acc
                }).apply(false, tree)

              val nameMap: mutable.Map[Name, Name] = mutable.Map()
              val evidenceParams: mutable.ListBuffer[Tree] = mutable.ListBuffer()

              def substMap(original: Tree, proto: Tree): Boolean = (original, proto) match {
                case (tpt1: Ident, tpt2: Ident) if tpt1.name == tpt2.name =>
                  true
                case (AppliedTypeTree(tpt1: Ident, args1), AppliedTypeTree(tpt2: Ident, args2))
                    if tpt1.name == tpt2.name && args1.forall(usesTparams) =>
                  (args1, args2).zipped.foreach {
                    case (arg1: Ident, arg2: Ident) =>
                      nameMap += arg1.name -> arg2.name
                    case (arg1, arg2: Ident) =>
                      evidenceParams += InfixOp(arg2, Ident(tpnme.<:<), arg1)
                    case _ =>
                  }
                  true
                case _ =>
              ...
              if (!substMap(selfParam.tpt, appliedTparamRef))
                Nil
              else {
                // type parameters of `def <opName>`
                val opTparams = tree.tparams.filterNot(tparam => nameMap.contains(tparam.name))

                val renamer = new UntypedTreeMap {
                  override def transform(tree: Tree)(implicit ctx: Context): Tree = tree match {
                    case tree: NameTree if nameMap.contains(tree.name) =>
                      rename(tree, nameMap(tree.name))
                    case _ =>
                ...
                val extraEvidence = makeImplicitParameters(evidenceParams.toList)

                // parameters of `def <opName>`
                val opVparamss = addEvidenceParams(otherParamss.mapconserve(renameTrees), extraEvidence)
                ...
                val selfArg =
                  if (extraEvidence.isEmpty)
                    ...
                    TypeApply(Select(Ident(nme.self), nme.asInstanceOf_), List(selfParam.tpt))

                val instanceArgss = (rest1 :: rest2).mapconserve(renameTrees) match {
                  case x :: xs =>
                    (selfArg :: x.map(v => Ident(v.name))) :: xs.nestedMap(v => Ident(v.name))
                  case Nil =>
                    ???
                }
