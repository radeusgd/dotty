...
export class DottyDebugConfigurationProvider implements vscode.DebugConfigurationProvider {
    ...
    private constructLaunchConfigName(mainClass: string, projectName: string, cache: { [name: string]: number }) {
...
async function updateDebugSettings() {
    const debugSettingsRoot: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration("dotty.debug");
...
function convertLogLevel(commonLogLevel: string) {
    // convert common log level to dotty log level
